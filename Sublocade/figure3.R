...
data <- data.frame(
Parameter    = c('Fatal OD Rate', 'Proportion Male', 'Proportion with IDU at Baseline', 'Proportion with ADU at Baseline', 'Background Mortality'),
...
g2 <- df2 %>%
  ...
  labs(y = "Quality Adjusted Life Years")
...
g_final <- ggdraw(add_sub(plot_grid(g1, g2 + 
          ...
          rel_widths = c(1,1,.2),
          labels = c("A","" ,"B")),
        "Fig 3A. Sensitivity of Cost for Extended Release Buprenorphine (XR-BUP) Strategy\n
        Fig 3B. Sensitivity of Quality-Adjusted Life Years for XR-BUP Strategy",
        size = 10,
        vjust = .4))

ggsave(plot = g_final, "figure3_v1.png")
